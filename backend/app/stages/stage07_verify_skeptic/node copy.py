...
Output state keys:
    - verdict_skeptic: DraftVerdict dict
"""
...
def run(state: dict) -> dict:
    ...
    logger.info(f"[{trace_id}] Stage7 시작: claim={claim_text[:50]}...")

    # 증거가 없으면 바로 UNVERIFIED
    if not evidence_topk:
        logger.warning(f"[{trace_id}] 증거 없음, UNVERIFIED 반환")
        state["verdict_skeptic"] = create_fallback_verdict("증거가 제공되지 않음")
        return state
    ...
    try:
        ...
        # 프롬프트 템플릿 로드
        prompt_path = pathlib.Path(__file__).parent / "prompt_skeptical.txt"
...
"""
        ...
        # SLM 호출
        logger.debug(f"[{trace_id}] Calling SLM2 for skeptical verification")
        ...
        # perspective 추가
        verdict["perspective"] = "skeptical"

        logger.info(
            f"[{trace_id}] Stage7 완료: stance={verdict.get('stance')}, "
