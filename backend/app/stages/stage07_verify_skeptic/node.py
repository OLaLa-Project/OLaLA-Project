"""
Stage 7 - Skeptical Verification (?뚯쓽??愿??寃利?

二쇱옣???뚯쓽?곸씤 愿?먯뿉??利앷굅瑜?遺꾩꽍?⑸땲??
...
## ?붿껌
??利앷굅瑜?諛뷀깢?쇰줈 二쇱옣??**?뚯쓽?곸씤 愿??*?먯꽌 遺꾩꽍?섍퀬, 吏?뺣맂 JSON ?뺤떇?쇰줈 寃곌낵瑜?異쒕젰?섏꽭??
?몄뼱: {language}
"""
...
def run(state: dict) -> dict:
    ...
    logger.info(f"[{trace_id}] Stage7 ?쒖옉: claim={claim_text[:50]}...")

    # 利앷굅媛 ?놁쑝硫?諛붾줈 UNVERIFIED
    if not evidence_topk:
        logger.warning(f"[{trace_id}] 利앷굅 ?놁쓬, UNVERIFIED 諛섑솚")
        state["verdict_skeptic"] = create_fallback_verdict("利앷굅媛 ?쒓났?섏? ?딆쓬")
        return state
    ...
    try:
        ...
        logger.info(
            f"[{trace_id}] Stage7 ?꾨즺: stance={verdict['stance']}, "
