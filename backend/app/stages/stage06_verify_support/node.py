...
def _stage06_diagnostics(
    *,
    ...
    return {
        ...
        "required_intent_count": required_intent_count,
...
def run(state: dict) -> dict:
    ...
    support_pool = state.get("evidence_topk_support", [])
    shared_pool = state.get("evidence_topk", [])
    if isinstance(support_pool, list) and support_pool:
        evidence_topk = support_pool
        input_pool_type = "support"
    else:
    ...
    logger.info(
        "[%s] Stage6 시작: claim=%s..., mode=%s, required_intents=%s",
        trace_id,
        claim_text[:50],
        claim_mode,
    ...
    if not evidence_topk:
        ...
        state["stage06_diagnostics"] = _stage06_diagnostics(
            mode=claim_mode,
            input_pool_type=input_pool_type,
            input_pool_avg_trust=input_pool_avg_trust,
    ...
    if claim_mode in {"rumor", "mixed"} and not has_required_intent:
        logger.info("[%s] Stage6 단락: rumor/mixed 모드에서 required intent 근거 없음", trace_id)
    ...
    state["prompt_support_user"] = user_prompt
    state["prompt_support_system"] = system_prompt
    ...
    try:
        ...
        state["slm_raw_support"] = last_response
        if (not raw_verdict) and (last_response or "").strip():
            ...
            if recovered:
                ...
                logger.warning(
                    "[%s] Stage6 partial JSON 복구 적용: keys=%s",
                    trace_id,
        ...
        logger.info(
            "[%s] Stage6 완료: stance=%s, confidence=%.2f, citations=%d",
            trace_id,
            verdict.get("stance"),
        ...
        logger.exception("[%s] Stage6 예상치 못한 오류: %s", trace_id, e)
        state["slm_raw_support"] = last_response
    ...
    state["stage06_diagnostics"] = _stage06_diagnostics(
        ...
        input_pool_type=input_pool_type,
        input_pool_avg_trust=input_pool_avg_trust,
        required_intent_count=len(required_evid_ids),
