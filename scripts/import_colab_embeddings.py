...
def main() -> None:
    ...
    # PERFORMANCE HACK: Disable synchronous commit for bulk loading speed
    ...
    try:
        ...
        print("Processing in batches (Insert -> Update -> Commit)...")
        # Reuse the batch size from env, default 5000
        for rows in _iter_rows(parquet_file, BATCH_SIZE, input_embed_col):
            ...
            # 2. Update target table immediately using the batch in temp
            cur.execute(
                f"""
            ...
            # 3. Commit ensuring data is saved incrementally
            conn.commit()
            
            # 4. Clear temp table for next batch (handled by ON COMMIT DELETE ROWS, but let's be explicit if needed)
            # Since we committed, ON COMMIT DELETE ROWS triggers.
            
            print(f"  Processed batch of {len(rows)} | Updated {affected} rows | Total: {total_updated}")

        print(f"Comparison finished in {time.time() - insert_started_at:.2f}s")
