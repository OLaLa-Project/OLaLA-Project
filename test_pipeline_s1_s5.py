...
def test_pipeline_s1_s5():
    ...
    print(f"INFO:test_pipeline:Testing Pipeline Stages 1-5 for URL: {url}")
    print("="*60)
    ...
    try:
        ...
        print("\n" + "="*60)
        print(" [VERIFICATION] Stage 5 (TopK / Rerank) ")
        print("="*60)
        topk = final_state.get('evidence_topk') or []
        print(f"TopK Selected: {len(topk)}")
        
        for i, item in enumerate(topk):
            print(f"\n[Rank {i+1}] Score: {item.get('score', 0.0):.4f} | Type: {item.get('source_type')}")
